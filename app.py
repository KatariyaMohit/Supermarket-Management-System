...
# Route to manage products in a block, based on block_id (VARCHAR)
@app.route('/block_manage_products', methods=['GET'])
def block_manage_products():
    block_incharge_id = session.get('user_id')
    connection = get_db_connection()
    cursor = connection.cursor(dictionary=True)

       # Fetch block name for the logged-in block manager
    block_query = '''
        SELECT block_name
        FROM Block
        WHERE block_incharge_id = %s;
    '''
    cursor.execute(block_query, (block_incharge_id,))
    block = cursor.fetchone()

    
    query = '''
        SELECT p.product_id, p.product_type, p.brand, p.cost_price, p.weight,
               p.selling_price, p.category_id, p.offer_id, p.block_count, p.warehouse_count
        FROM product p
        JOIN category c ON p.category_id = c.category_id
        JOIN block b ON c.store_id = b.block_id
        WHERE b.block_incharge_id = %s;
    '''
    
    cursor.execute(query, (block_incharge_id,))
    block_products = cursor.fetchall()
    
    cursor.close()
    connection.close()
    
    return render_template('block_manage_products.html', products=block_products,block_name=block['block_name'] if block else "Unknown Block")

@app.route('/edit_block_products', methods=['POST'])
def edit_block_products():
    product_ids = request.form.getlist('product_ids')
    products_to_edit = []
    ...
    return render_template('edit_block_products.html', products=products_to_edit, categories=categories, offers=offers)

# Update products
@app.route('/update_block_products', methods=['POST'])
def update_block_products():
    for key, value in request.form.items():
        ...
        if key.startswith('type-'):
           ...
           # cursor.execute("UPDATE Product SET block_count = %s WHERE product_id = %s", (value, product_id))
            ...
            cursor.execute("UPDATE Product SET warehouse_count = %s WHERE product_id = %s", (value, product_id))
    db.commit()
    return redirect(url_for('block_manage_products'))

# Get payment modes for the dropdown
@app.route('/get_payment_modes')
def get_payment_modes():
    ...
    payment_modes = cursor.fetchall()
    conn.close()
    return jsonify({'paymentModes': payment_modes})

# Get products and apply offer discounts
@app.route('/get_products')
def get_products():
    conn = get_db_connection()
    cursor = conn.cursor(dictionary=True)
    cursor.execute("""
        SELECT p.product_id, p.product_type, p.selling_price, p.offer_id, o.offers_type
        FROM Product p
        LEFT JOIN OfferDetails o ON p.offer_id = o.offers_id
    """)
    products = cursor.fetchall()
...
def save_products():
    ...
    for product in products:
        ...
        # Insert into buys_products table
        cursor.execute("""
...
# Route to get customer email
@app.route('/get_customer_email', methods=['GET'])
def get_customer_email():
    customer_name = request.args.get('name')
    print(f"Received customer name: {customer_name}")

    cursor = db.cursor(dictionary=True)
    cursor.execute("SELECT email FROM Customer WHERE customer_name = %s", (customer_name,))
    customer = cursor.fetchone()
    print(f"Query result: {customer}")
    cursor.close()

    if customer:
        return jsonify({'email': customer['email']})
    else:
        return jsonify({'email': None}), 404

# Route to send the bill email
@app.route('/send_bill', methods=['POST'])
def send_bill():
    try:
        print("Received request to /send_bill")

        customer_name = request.form.get('customer_name')
        customer_email = request.form.get('customer_email')
        bill_details = request.form.get('bill_details')

        print(f"Form data: name={customer_name}, email={customer_email}, bill={bill_details}")

        if not all([customer_name, customer_email, bill_details]):
            return "Missing required form fields.", 400

        feedback_link = f'http://yourwebsite.com/feedback?customer_email={customer_email}'

        subject = f"Your Bill from Our Supermarket, {customer_name}"
        body = f"""
        <html>
        <body>
        <p>Dear {customer_name},</p>
        <p>Thank you for shopping with us! Here is your bill:</p>
        <div>{bill_details}</div>
        <p>We value your feedback! Please click the link below to provide feedback:</p>
        # <p><a href="{feedback_link}">Give Feedback</a></p>
        <p>Best regards,<br>Your Supermarket Team</p>
        </body>
        </html>
        """
        ...
        print(f"Mail sent successfully to {customer_email}")
        return f"Bill sent to {customer_email} successfully!"
